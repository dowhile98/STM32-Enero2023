 ...
 */
/**
 * en este proyecto se hace un pequeño middleware para manejo de dispositivos AT
 * el dispositivo utilizando es un TinyFOX
 */
#include <stdint.h>
#include <stm32f4xx.h>
#include <stdio.h>
...
uint8_t responsedata[20];			//para analizar la respuesta

uint8_t data[100];				//buffer para almacenar los datos recibidos
lwrb_t rbbuff = {0};			//buffer circular
/****************************************************************************/
/**
...
/**
 * @brief funcion que se ejecuta cuando se escribe/lee o reinicia el buffer circular
 */
void rb_evt_fn(lwrb_t* buff, lwrb_evt_type_t type, size_t len);
/**
 * @brief funcion para verificar la respuesta del dispositivo AT
 * @param [respuesta esperada]
 * @param [timout] tiempo maximo de espera por la respuesta
 * @return bool
 */
bool at_verify_response(uint8_t *responsecheck, uint32_t timout);

/**
 * @brief funcion para enviar comandos al dispositivo AT
 */
...
int main(void)
{
	...
	GPIOX_ODR(LED2)   = 1;
	if(at_verify_response((uint8_t*)"OK\r\n", 10000)){ //se verifica la respuesta
		printf("el dispositivo respondio\r\n");			//se muestra un mensaje
		delay_ms(100);
		//se lee la bateria
		at_send_cmd((uint8_t*)"AT$V?\r\n", 3000);		//se envia el comando para leer la bateria
		delay_ms(500);
		lwrb_read(&rbbuff, responsedata, 7);			//se lee la bateria del buffer
	...
	GPIOX_ODR(LED2)   = 0;
	//...
	//Se pondrá la guia de los comandos AT del dispositivo ussado
